...
- UI текст — на русском
- **Минимализм** — НИКАКИХ эмодзи без явной просьбы
- НИКАКИХ SVG-иконок без явной просьбы
- Сохранять пропорции — менять только то, о чём попросили

### Дизайн страниц
- Страницы со скроллом по умолчанию, если пользователь явно не попросит без скролла
- Учитывать content padding (`base.html .content { padding: 28px }`)
- Разделители единообразные — `border-bottom: 1.5px solid var(--border-color)`

### Git
- НИКОГДА не коммитить без явной просьбы
- НИКОГДА не пушить в remote без явной просьбы

### Docker
- Restart: `docker-compose -f docker-compose.yml restart`
- Изменения шаблонов Django НЕ требуют rebuild — достаточно перезапуска или обновления браузера

### Код
- Следовать существующей дизайн-системе (CSS переменные, Inter font, radius-sm/md/lg)
- Не вводить новые паттерны без просьбы
- При редактировании CSS менять только то, о чём попросили

### Коммуникация
- Отвечать кратко, на языке пользователя (предпочтительно русский)
- Не объяснять что было сделано — просто подтвердить
